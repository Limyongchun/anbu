      ...
      {!immersive && hasParents && (
        <View style={[mp.summaryCardsWrap, { bottom: BOTTOM_SAFE + 14 }]}>
          <ScrollView
            ...
            contentContainerStyle={mp.summaryCardsScroll}
            pagingEnabled={false}
          ...
          >
            {parentLocs.map((pl, i) => {
              ...
              return (
                <Pressable
                  key={pl.deviceId}
                  style={[mp.summaryCard, isSel && mp.summaryCardSelected]}
                  onPress={() => selectParent(i)}
                >
                  <View style={mp.summaryCardTop}>
                    <View style={mp.summaryCardNameRow}>
                      <View style={[mp.summaryDot, { backgroundColor: freshColor }]} />
                      <Text style={mp.summaryName} numberOfLines={1}>{pl.memberName}</Text>
                    </View>
                    <Text style={[mp.summaryStatus, { color: isMoving ? DS.success : DS.textSecondary }]}>
                      {statusLabel}
                    </Text>
                  </View>

                  <Text style={mp.summaryTime}>{timeLabel}</Text>

                  {diffMin <= 5 && (
                    <Text style={mp.summaryCalm}>{t.mapCalm}</Text>
                  )}

                  <View style={mp.summaryActions}>
                    <Pressable style={mp.summaryActionBtn} onPress={() => Linking.openURL("tel:")}>
...
function HomeScreen({
  ...
  const parentStatusResults = useMemo(() => {
  ...
  useEffect(() => {
    ...
    parentStatusResults.forEach((ps: ParentStatusInfo) => {
      ...
      const curr = ps.result.level;
      if (prev && prev !== curr) {
        const code = allFamilyCodes[0];
        api.logStatusChange(code, {
          deviceId: ps.deviceId,
          parentName: ps.parentName,
          previousStatus: prev,
          newStatus: curr,
          place: ps.result.place,
          reason: ps.result.isSleeping ? "sleep" : undefined,
        }).catch(() => {});
      }
      prevStatusRef.current[key] = curr;
    });
  }, [parentStatusResults, allFamilyCodes]);
    ...
    >
      ...
      {(() => {
        ...
        const sorted = [...parentActivityStats].sort((a, b) => {
          const sa = getParentStatusNew(a.deviceId, a.name).level;
          const sb = getParentStatusNew(b.deviceId, b.name).level;
...
const mp = StyleSheet.create({
  ...
  summaryCardSelected: { borderColor: DS.brand, borderWidth: 1.5 },
  summaryCardTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 8 },
  summaryCardNameRow: { flexDirection: "row", alignItems: "center", gap: 8 },
  summaryDot: { width: 10, height: 10, borderRadius: 5 },
  summaryName: { fontFamily: "Inter_700Bold", fontSize: 17, color: DS.textPrimary },
  summaryStatus: { fontFamily: "Inter_600SemiBold", fontSize: 12 },
  summaryTime: { fontFamily: "Inter_400Regular", fontSize: 13, color: DS.textTertiary, marginBottom: 4 },
  summaryCalm: { fontFamily: "Inter_500Medium", fontSize: 13, color: DS.success, marginBottom: 6 },
  summaryActions: { flexDirection: "row", gap: 16, marginTop: 8, paddingTop: 10, borderTopWidth: 1, borderTopColor: "rgba(0,0,0,0.05)" },
  summaryActionBtn: { flexDirection: "row", alignItems: "center", gap: 6 },
  summaryActionLabel: { fontFamily: "Inter_600SemiBold", fontSize: 13 },
