...
- **Heading/Direction**: DB `family_locations` stores `heading` (real, degrees) + `speed` (real, m/s); map markers show directional arrow when heading available; arrow pulses green when moving; speed badge shows km/h; banner shows "남동 방향 · 4 km/h" style info
